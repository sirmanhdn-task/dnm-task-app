...
function computeScore(task) {
    ...
    let score;
    if (t <= 5) score = state.quickBoost;
    else score = t / timeLeft;
...
function renderTasks() {
    ...
    sorted.forEach(t => {
        ...
        const hasConstraint = t.weekdays?.length > 0 || t.sessions?.length > 0;
        ...
        div.innerHTML = `
            ...
            <div class="task-footer">
                ...
                ${t.done ? "" : `<button class="calendar-btn">Calendar</button>`}
                ${t.done ? "" : `<button class="calendar-open-btn">OpenCal</button>`}
                ${t.done ? "" : `<button class="done-btn" style="background:#009688">Done</button>`}
